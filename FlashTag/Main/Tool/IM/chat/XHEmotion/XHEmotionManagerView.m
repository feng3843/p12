...
- (void)reloadData {
    ...
    CDEmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:self.selectedIndex];
...
- (void)setup {
    ...
    if (!_emotionCollectionView) {
        UICollectionView *emotionCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kXHEmotionPageControlHeight) collectionViewLayout:[[XHEmotionCollectionViewFlowLayout alloc] init]];
    ...
    if (self.isShowEmotionStoreButton) {
        view = self.emotionPageControl;
        view.backgroundColor = [UIColor colorWithWhite:0.886 alpha:1.000];
    }
    else
    {
        view = self.emotionCollectionView;
        view.backgroundColor = [UIColor clearColor];
    }

    if (!_emotionSectionBar) {
        XHEmotionSectionBar *emotionSectionBar = [[XHEmotionSectionBar alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(view.frame), CGRectGetWidth(self.bounds), kXHEmotionSectionBarHeight) showEmotionStoreButton:self.isShowEmotionStoreButton];
        emotionSectionBar.delegate = self;
        emotionSectionBar.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        emotionSectionBar.backgroundColor = view.backgroundColor;
...
- (void)didSelecteEmotionManager:(CDEmotionManager *)emotionManager atSection:(NSInteger)section {
...
-(void)didSendEmotionButton:(id)button
{
    if([self.delegate respondsToSelector:@selector(didSendEmotionButton:)]){
        [self.delegate didSendEmotionButton:button];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(didSelecteEmotion:atIndexPath:)]) {
        CDEmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:indexPath.section];
