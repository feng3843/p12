...
#define kXHEmotionStoreButtonMargin 0
...
{
    if([self.delegate respondsToSelector:@selector(didSendEmotionButton:)]){
        [self.delegate didSendEmotionButton:sender];
    }
}

- (PYEmotionStoreItemButton *)createdButton {
    PYEmotionStoreItemButton *button = [PYEmotionStoreItemButton buttonWithType:UIButtonTypeCustom];
...
- (void)reloadData {
    ...
    for (CDEmotionManager *emotionManager in self.emotionManagers) {
        NSInteger index = [self.emotionManagers indexOfObject:emotionManager];
        PYEmotionStoreItemButton *sectionButton = (PYEmotionStoreItemButton*)[self createdButton];
        sectionButton.emotionStoreItemButtonStyle = PYEmotionStoreItemButtonStyleDefault;
...
- (void)setup {
    ...
    //TODO
    if (self.isShowEmotionStoreButton) {
        //[sectionButton addTarget:self action:@selector(storeButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    }
    UIButton *emotionSendButton = [self createdButton];

    CGRect emotionSendButtonFrame = emotionSendButton.frame;
    emotionSendButtonFrame.origin.x = CGRectGetWidth(self.bounds) - kXHStoreManagerItemWidth - kXHEmotionStoreButtonMargin;
    emotionSendButton.frame = emotionSendButtonFrame;

    [emotionSendButton setTitle:@"发送" forState:UIControlStateNormal];
    [emotionSendButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    emotionSendButton.backgroundColor = [UIColor redColor];
    [emotionSendButton addTarget:self action:@selector(sendButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:emotionSendButton];
    _storeManagerItemButton = emotionSendButton;
}
