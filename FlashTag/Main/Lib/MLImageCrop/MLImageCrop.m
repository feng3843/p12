...
//    buttonBackgroundView.layer.opacity = 0.8f;
    ...
    NSString* strNext;
    //绘制俩button
    [self.view addSubview:self.cancelButton = [self acquireCustomButtonWithTitle:@"" andAction:@selector(onCancel:)]];
    switch (self.type) {
        case CutTypeCircle:
        {
            strNext = @"保存";
    ...
    [self.view addSubview:self.confirmButton = [self acquireCustomButtonWithTitle:strNext andAction:@selector(onConfirm:)]];
    
    [self.cancelButton setImage:[UIImage imageNamed:@"ic_home_mark_back"] forState:UIControlStateNormal];
    [self.confirmButton setBackgroundImage:[UIImage resizedImage:@"bt_home_normal_somewhere"] forState:UIControlStateNormal];
    [self.confirmButton setBackgroundImage:[UIImage resizedImage:@"bt_home_press_somewhere"] forState:UIControlStateSelected];
    //[self.confirmButton setBackgroundImage:[UIImage resizedImage:@"bt_home_press_somewhere"] forState:UIControlStateHighlighted];
...
- (void)showWithAnimation:(BOOL)animation
{
    ...
    if (animation) {
        CGRect bounds = self.actionWindow.layer.bounds;
        bounds.origin.x = - self.actionWindow.layer.bounds.size.width;
        self.actionWindow.layer.bounds = bounds;
...
//        self.actionWindow.layer.opacity = 0.01f;
        ...
        if (isNext) {
            switch (self.type) {
                case CutTypeCircle:
                {
                    bounds.origin.y = - self.actionWindow.layer.bounds.size.height;
                }
                    break;
                default:
                {
...
- (UIView*)overlayView
{
    if (!_overlayView) {
        ...
        switch (self.type) {
            ...
            default:
            {
                _overlayView.layer.borderWidth = 0.0f;
            }
                break;
        }
...
#define kButtonViewHeight 75
    ...
    //底部俩按钮
#define kButtonHeight_2 30
#define kButtonWidth_2 73
#define kButtonLeft (20 - 6)//6:按钮左侧
#define kButtonRight 15
    self.cancelButton.frame = CGRectMake(kButtonLeft, CGRectGetMinY(self.buttonBackgroundView.frame)+(CGRectGetHeight(self.buttonBackgroundView.frame) - 24)/2, 24, 24);
    self.confirmButton.frame = CGRectMake(CGRectGetWidth(self.buttonBackgroundView.frame)-kButtonWidth_2-kButtonRight, CGRectGetMinY(self.buttonBackgroundView.frame)+(CGRectGetHeight(self.buttonBackgroundView.frame) - kButtonHeight_2)/2, kButtonWidth_2, kButtonHeight_2);
    ...
    switch (self.type) {
        case CutTypeCircle:
        {
            self.overlayView.frame = CGRectMake(0, OVERLAY_CIRCLE_Y, height, height);
            self.overlayView.center = CGPointMake(self.view.bounds.size.width/2, self.overlayView.center.y);
            self.overlayView.layer.cornerRadius = CGRectGetHeight(self.overlayView.frame)/2;
            
            
            circlePath  = [UIBezierPath bezierPathWithOvalInRect:self.overlayView.frame];
        }
            break;
        default:
        {
            self.overlayView.frame = CGRectMake(0, OVERLAY_Y, width, height);
            //    self.overlayView.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height/2);
            circlePath = [UIBezierPath bezierPathWithRect:self.overlayView.frame];
        }
            break;
    }
    {
        ...
        UIView *plCropOverlay = self.overlayView;
        ...
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), screenHeight)];
        ...
        fillLayer.opacity = 0.8f;
        [self.topBlackView.layer addSublayer:fillLayer];
    }
    
    //上下黑色覆盖View
    if (isBaseOnWidth) {
        //上和下
        self.topBlackView.frame = self.scrollView.frame;
    ...
    if (self.imageView.image) {
        ...
        if (isBaseOnWidth) {
            ...
            switch (self.type) {
                case CutTypeCircle:
                {
                    self.scrollView.contentInset = UIEdgeInsetsMake(CGRectGetMinY(self.overlayView.frame),
                                                                    CGRectGetMinX(self.overlayView.frame),
                                                                    CGRectGetHeight(self.view.bounds)-CGRectGetMaxY(self.overlayView.frame), CGRectGetWidth(self.view.bounds)-CGRectGetMaxX(self.overlayView.frame));
                }
                    break;
        ...
        if (isBaseOnWidth) {
            ...
            switch (self.type) {
                case CutTypeCircle:
                {
                    ...
                    CGFloat offsetY = CGRectGetMinY(self.overlayView.frame);
